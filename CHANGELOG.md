...
### Breaking Changes
- `Attribute.type_` is now `StandardApi.Type.Type` instead of `String`.
...
- `modelToTypes` helper function to convert schema `Model` to type mappings for
  the parser.
- `Decimal` variant to `Value` type for preserving numeric precision on decimal,
  numeric, and money fields (avoids IEEE 754 rounding).
